...
class AirSimDroneEnvSAC(gym.Env):
    def __init__(self, ip_address, image_shape, env_config):
        self.image_shape = image_shape
        self.sections = env_config["sections"]
    ...
    def do_action(self, select_action):
        speed = 0.4
        vx = speed
        vy = float(select_action[0]) * speed
        vz = float(select_action[1]) * speed

        self.drone.moveByVelocityBodyFrameAsync(vx, vy, vz, duration=self.dt).join()

        # Execute action
        # print(f"Action taken: vx={speed}, vy={vy}, vz={vz}")
...
class AirSimDroneEnvDQN(gym.Env):
    def __init__(self, ip_address, image_shape, env_config):
        self.image_shape = image_shape
        self.sections = env_config["sections"]
